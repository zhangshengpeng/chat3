...
export default {
  ...
  methods: {
    handelclick(item,i) {
      ...
      this.$axios.post('http://www.bixiaohe.fun/history',
      {
        userId: this.user.id,
        friendId: item.id
      })
    ...
    getInfo() {
      const _this = this
      this.$axios.get('http://www.bixiaohe.fun/user-info')
      .then(function (response) {
        if(response.status == 1) {
          ...
          _this.$axios.post('http://www.bixiaohe.fun/friend-info')
          .then(function(response) {
    ...
    operation(id) {
      if(id==='check'){
        console.log(id)
          ...
          this.$axios.post('http://www.bixiaohe.fun/friend',{
          friend_id:id,
    ...
    uploadSubmit() {
      this.$axios.post('http://www.bixiaohe.fun/upload-info',this.upload)
        .then((res) => {
